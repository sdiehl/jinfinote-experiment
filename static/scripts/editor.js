// Adapted from jinfinote demo , now uses ZeroMQ + Django +
// Gevent instead of Twisted: See original at:
//
// https://github.com/sveith/jinfinote/tree/master/demo
//
...
function CollaborativeEditor(ctl, url, session_id) {
    ...
    socket.on("connect", function connector (event) {
        ce._onSocketOpen(event);
        console.log("Socket connected");
    });

    socket.on("message", function(data) {
        ce._onSocketMessage(data);
        console.log("Message received");
    });

    socket.on("close", function(data) {
        ce._onSocketConnectionLost();
    });
...
CollaborativeEditor.prototype._updateControl = function(executedRequest) {
    ...
    if(executedRequest === undefined ||
       executedRequest.operation === undefined) {
        return;
    }
...
CollaborativeEditor.prototype._onSocketMessage = function(data) {
